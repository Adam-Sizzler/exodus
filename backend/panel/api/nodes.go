...
func scanNode(scanner RowScanner) (Node, error) {
	...
	err := scanner.Scan(
		...
		&notifyPercent, &providerUUID, &n.ViewPosition, &n.CountryCode, &tags,
...
func NodesReorderHandler(manager *dbmanager.DatabaseManager, cfg *config.BackendConfig) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
...
func sendNodesHelp(w http.ResponseWriter) {
	help := map[string]interface{}{
		...
		"response_fields": map[string]string{
			...
			"port":                       "Node gRPC port",
			"api_schema":                 "API protocol (grpc, grpcs, https, http, tls)",
...
func handleCreateNode(w http.ResponseWriter, r *http.Request, manager *dbmanager.DatabaseManager, cfg *config.BackendConfig) {
	...
	err := manager.ExecuteHighPriority(func(db dbmanager.DBExecutor) error {
		...
		_, err := db.ExecContext(ctx, query,
			...
			req.ViewPosition, req.CountryCode, tags)
...
func handleDeleteAllNodes(w http.ResponseWriter, r *http.Request, manager *dbmanager.DatabaseManager, cfg *config.BackendConfig) {
	ctx := r.Context()
...
func (r *NodeUpdateRequest) Validate() error {
	...
	if r.APISchema != nil {
		...
		case "grpc", "grpcs", "https", "http", "tls":
		default:
			return fmt.Errorf("api_schema must be one of: grpc, grpcs, https, http, tls")
		}
	}
	return nil
}
...
func handleGetNodesSummary(w http.ResponseWriter, r *http.Request, manager *dbmanager.DatabaseManager, cfg *config.BackendConfig) {
	ctx := r.Context()
	...
	err := manager.ExecuteHighPriority(func(db dbmanager.DBExecutor) error {
		...
		for rows.Next() {
			...
			err := rows.Scan(
				...
				&s.TrafficLimitRaw, &s.CountryCode, &tags, &s.LastStatusMsg, &s.CreatedAt,
			)
			if err != nil {
				return err
			}
...
func NodesSummaryHandler(manager *dbmanager.DatabaseManager, cfg *config.BackendConfig) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
