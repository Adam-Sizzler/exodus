...
func PanelSettingsHandler(manager *manager.DatabaseManager, cfg *config.BackendConfig) http.HandlerFunc {
		...
		case http.MethodPatch, http.MethodPut:
			...
			err := manager.ExecuteHighPriority(func(db *sql.DB) error {
				if _, execErr := db.Exec(`
					INSERT OR IGNORE INTO remnawave_settings (
				...
				`,
					...
					`{"enabled":true}`,
					`{"title":"V2RayStat","logoUrl":null}`,
				); execErr != nil {
					return execErr
				}

				query := "UPDATE remnawave_settings SET " + strings.Join(setClauses, ", ") + " WHERE id = 1"
...
func PanelAPITokensHandler(manager *manager.DatabaseManager, cfg *config.BackendConfig) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		switch r.Method {
		case http.MethodGet:
...
func PanelAPITokenByUUIDHandler(manager *manager.DatabaseManager, cfg *config.BackendConfig) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
...
func loadPanelSettings(manager *manager.DatabaseManager) (map[string]any, error) {
	settings := map[string]any{
		...
		"branding_settings": map[string]any{"title": "V2RayStat", "logoUrl": nil},
	}
...
func loadAPITokens(manager *manager.DatabaseManager) ([]APITokenRecord, error) {
