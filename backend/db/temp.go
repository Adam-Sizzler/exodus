...
// Глобальные previous для агрегации (user/source -> [uplink, downlink])
var (
...
// MonitorHourlyAggregation запускает рутину для hourly агрегации каждые 10 мин
func MonitorHourlyAggregation(ctx context.Context, manager *manager.DatabaseManager, cfg *config.BackendConfig, wg *sync.WaitGroup) {
	go func() {
		defer wg.Done()
		ticker := time.NewTicker(10 * time.Minute)
		defer ticker.Stop()
		cfg.Logger.Debug("Starting hourly aggregation monitoring")
		for {
			select {
			case <-ticker.C:
			...
			case <-ctx.Done():
				cfg.Logger.Debug("Stopped hourly aggregation monitoring")
				return
			}
		}
	}()
