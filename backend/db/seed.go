...
func ensureV2rsSettings(ctx context.Context, tx *sql.Tx) error {
	var exists int
	if err := tx.QueryRowContext(ctx, `SELECT COUNT(*) FROM v2rs_settings WHERE id = 1`).Scan(&exists); err != nil {
	...
	`)
	if _, err := tx.ExecContext(ctx, query, passkeySettings, oauth2Settings, tgAuthSettings, passwordSettings, brandingSettings); err != nil {
