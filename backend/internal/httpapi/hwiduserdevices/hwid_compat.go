...
func handleHWIDCompatCreateUserDevice(w http.ResponseWriter, r *http.Request, manager *dbmanager.DatabaseManager, cfg *config.BackendConfig) {
	...
	err := manager.ExecuteHighPriority(func(db dbmanager.DBExecutor) error {
		...
		if err := db.QueryRowContext(r.Context(), `SELECT EXISTS(SELECT 1 FROM hwid_user_devices WHERE hwid = ? AND user_uuid = ?)`, hwid, userUUID).Scan(&deviceExists); err != nil {
			return err
		}
		if deviceExists {
		...
		_, err := db.ExecContext(r.Context(), `
			INSERT INTO hwid_user_devices (hwid, user_uuid, platform, os_version, device_model, user_agent, created_at, updated_at)
			VALUES (?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
		`, hwid, userUUID, req.Platform, req.OSVersion, req.DeviceModel, req.UserAgent)
		return err
	})
	if err != nil {
		switch {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		err := manager.ExecuteHighPriority(func(db dbmanager.DBExecutor) error {
			...
			for rows.Next() {
				...
				byPlatform = append(byPlatform, map[string]any{"platform": platform, "count": count})
			}
			...
			rows2, err := db.QueryContext(r.Context(), `
				SELECT SPLIT_PART(user_agent, '/', 1) AS app, COUNT(*) AS count
				FROM hwid_user_devices
			...
			for rows2.Next() {
				var app string
				var count int
				if scanErr := rows2.Scan(&app, &count); scanErr != nil {
					return scanErr
				}
				if strings.HasPrefix(app, "https:") {
					continue
				}
				byApp = append(byApp, map[string]any{"app": app, "count": count})
			}
			return rows2.Err()
		})
		if err != nil {
...
func getHWIDCompatDevices(ctx context.Context, manager *dbmanager.DatabaseManager, start, size int, r *http.Request) ([]hwidCompatDevice, int, error) {
	...
	err := manager.ExecuteHighPriority(func(db dbmanager.DBExecutor) error {
		countQuery := `SELECT COUNT(*) FROM hwid_user_devices h` + whereSQL
...
func scanHWIDCompatDevice(scanner hwidCompatScanner) (hwidCompatDevice, error) {
	...
	if err := scanner.Scan(&item.HWID, &item.UserUUID, &item.Platform, &item.OSVersion, &item.DeviceModel, &item.UserAgent, &createdAt, &updatedAt); err != nil {
		return item, err
	}
	if createdAt.Valid {
