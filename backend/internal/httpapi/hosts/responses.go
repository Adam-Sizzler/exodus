...
func mapHostRecordToAPI(rec hostRecord, nodes []string, squads []string) HostAPI {
	mode := rec.InternalSquadsMode
	if mode == "" {
		mode = "EXCLUDE"
	}
	squadList := ensureStringSlice(squads)
	return HostAPI{
		UUID:                   rec.UUID,
		...
		InternalSquads:         HostInternalSquads{Mode: mode, Squads: squadList},
		ExcludedInternalSquads: squadList,
