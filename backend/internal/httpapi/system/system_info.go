...
func readBuildMetadata() (version string, commitSHA string, branch string, buildTime string) {
	...
	if value := firstMetadataEnv("__EX_METADATA_VERSION", "EXODUS_VERSION"); value != "" {
		version = value
	}
	if value := firstMetadataEnv("__EX_METADATA_GIT_BACKEND_COMMIT", "EXODUS_BACKEND_COMMIT", "EXODUS_REVISION", "EXODUS_COMMIT", "GITHUB_SHA"); value != "" {
		commitSHA = value
	}
	if value := firstMetadataEnv("__EX_METADATA_GIT_BRANCH", "EXODUS_GIT_BRANCH", "EXODUS_BRANCH", "GITHUB_REF_NAME", "GITHUB_HEAD_REF"); value != "" {
		branch = value
	}
	if value := firstMetadataEnv("__EX_METADATA_BUILD_TIME", "EXODUS_BUILD_TIME"); value != "" {
		buildTime = value
	}
