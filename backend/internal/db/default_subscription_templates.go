...
// DefaultSubscriptionTemplate defines a built-in template used for seeding and create operations.
...
`),
		...
		},
		{
			...
			TemplateJSON: json.RawMessage([]byte(`{
  "log": {
    "disabled": true,
    "level": "debug",
    "timestamp": true
...
// DefaultSubscriptionTemplateByType returns a default template for the given type.
func DefaultSubscriptionTemplateByType(templateType string) (DefaultSubscriptionTemplate, bool) {
	for _, tmpl := range DefaultSubscriptionTemplates() {
		if tmpl.TemplateType == templateType {
			return tmpl, true
		}
	}
	return DefaultSubscriptionTemplate{}, false
}
