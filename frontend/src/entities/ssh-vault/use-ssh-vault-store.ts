...
const VAULT_AAD = 'ex-vault-v1'
const PASSCODE_AAD = 'ex-vault-passcode-v1'
...
function generatedKey(nodeUuid: string): INewNodeKey {
    ...
    return {
        ...
        publicKeyLine: toOpenSshPublicKey(pair.publicKey, `exodus:${nodeUuid}`)
    }
}
        ...
        (set, get) => ({
            ...
            actions: {
                ...
                importNodeKey: async (nodeUuid, privateKey) => {
                    const parsed = await parseSshPrivateKey(privateKey, `exodus:${nodeUuid}`)
