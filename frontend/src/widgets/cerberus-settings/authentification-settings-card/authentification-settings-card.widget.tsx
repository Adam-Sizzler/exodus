...
import {
    GetCerberusSettingsCommand,
    UpdateCerberusSettingsCommand
...
import { PasskeysDrawerComponent } from '@widgets/cerberus-settings/passkeys-settings-drawer/passkeys-drawer.component'
import { useUpdateCerberusSettings } from '@shared/api/hooks/cerberus-settings/cerberus-settings.mutation.hooks'
...
interface IProps {
    oauth2Settings: NonNullable<GetCerberusSettingsCommand.Response['response']['oauth2Settings']>
    passkeySettings: NonNullable<
        GetCerberusSettingsCommand.Response['response']['passkeySettings']
    >
    passwordSettings: NonNullable<
        GetCerberusSettingsCommand.Response['response']['passwordSettings']
    >
    tgAuthSettings: NonNullable<GetCerberusSettingsCommand.Response['response']['tgAuthSettings']>
}
...
export const AuthentificationSettingsCardWidget = (props: IProps) => {
    ...
    const form = useForm<NonNullable<UpdateCerberusSettingsCommand.Request>>({
        name: 'auth-settings',
        mode: 'uncontrolled',
        validate: zodResolver(
            UpdateCerberusSettingsCommand.RequestSchema.pick({
    ...
    const { mutate: updateSettings, isPending: isUpdatePending } = useUpdateCerberusSettings({
        mutationFns: {
            onSuccess(data) {
                queryClient.refetchQueries({
                    queryKey: QueryKeys.cerberusSettings.getCerberusSettings.queryKey
                })
