...
When a user requests a subscription – depending on the client application from which the request came, the user may receive different templates. For example, if the application runs on the Mihomo core – Exodus will detect this and provide the **Default** template of the **Mihomo** type. (You can manage templates in the corresponding section).

Inside Exodus, you can create an infinite number of templates for each type (Mihomo, Stash, Xray Json, Singbox, Clash), but by default, the **Default** template will always be provided.
...
For example, in general settings my profile header is set as "Exodus", but for 10 users I want to override it, for example to "Exodus v.2.x", in this case I override this parameter in this section, and then assign this external squad to the needed 10 users.
