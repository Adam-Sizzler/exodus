...
4. پیکربندی کنید:
   - **Client ID**: `exodus` (یا نام دلخواه شما)
...
یک claim سفارشی `exodusAccess: true` به توکن اضافه کنید:
...
به جای پیکربندی claim، می‌توانید لیستی از آدرس‌های ایمیل مجاز را در تنظیمات Exodus مشخص کنید. فقط کاربرانی که ایمیل آن‌ها در این لیست است می‌توانند وارد شوند.
...
| **Frontend Domain** | دامنه پنل Exodus بدون `https://` (مثلاً `panel.example.com`) |
...
- کاربر claim `exodusAccess: true` را از طریق mapper پیکربندی کرده است، یا
- ایمیل کاربر به لیست Allowed Emails در تنظیمات Exodus اضافه شده است
