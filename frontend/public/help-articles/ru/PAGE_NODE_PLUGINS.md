...
Модуль формирует таблицу учетных данных пользователей для быстрой авторизации в HAProxy через Lua-скрипты и автоматически обновляет ее при изменении конфигурации или пользователей через сокет без перезагрузки.
...
- В поле `inboundTags` можно указать `["*"]` для включения всех поддерживаемых инбаундов ноды или перечислить конкретные теги (например, `["vless-inbound", "trojan-inbound", "naive-inbound"]`).
- Создает и актуализирует файл `/opt/app/haproxy/data/users.csv` внутри контейнера ноды.
- Записывает строгий двухколоночный формат: `username,credential` (без устаревшего префикса 1/0).
- Поддерживает протоколы: `VLESS UUID`, `Trojan credential` (SHA-224 hash), `AnyTLS credential` (SHA-256 hash) и `NaiveProxy credential` (`basic:<base64(username:password)>`).
