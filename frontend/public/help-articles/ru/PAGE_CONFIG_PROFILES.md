...
Профиль, в контексте Exodus, представляет из себя полную серверную конфигурацию ядра [Xray](https://xtls.github.io/ru/config/).
...
На данный момент, Exodus поддерживает такие протоколы как: `VLESS`, `Trojan`, `Shadowsocks` (`chacha20-ietf-poly1305`, `2022-blake3-aes-256-gcm`, `aes-128-gcm`, `aes-256-gcm`), `Hysteria2` (только для клиентского Xray-Json). И так же следующие транспорты: `RAW (TCP)`, `XHTTP`, `Websocket`, `HTTPUpgrade`, `gRPC`, `KCP`.

Важно отметить, что Exodus так же поддерживает протоколы: `Tunnel`, `mixed(socks)`, `wireguard`,`http` – однако панель будет полностью их игнорировать и менеджмент пользователей для этих протоколов будет недоступен. Эти _инбаунды_ будет переданы в Xray в таком виде, в каком вы их укажите.

Для основных протоколов (`VLESS`, `Trojan`, `Shadowsocks`) Exodus будет заниматься управлением списка пользователей, которые будут внутри серверной конфигурации. С вашей стороны никаких дополнительных действий не требуется.
...
По умолчанию, Exodus автоматически добавляет flow для следующих конфигураций: VLESS+TLS/REALITY+RAW/TCP.
...
Так как Exodus необходимо генерировать клиентскую сторону – мы добавили кастомное поле, которого не существует в оригинальной конфигурации Xray.
