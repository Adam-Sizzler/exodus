...
4. Настройте:
   - **Client ID**: `exodus` (или другое имя)
...
Добавьте claim `exodusAccess: true` в токен:
...
Вместо настройки claim вы можете указать список разрешённых email-адресов в настройках Exodus. Только пользователи с email из этого списка смогут войти.
...
| **Frontend Domain** | Домен панели Exodus без `https://` (например, `panel.example.com`) |
...
- У пользователя настроен claim `exodusAccess: true` через маппер, ИЛИ
- Email пользователя добавлен в список Allowed Emails в настройках Exodus
