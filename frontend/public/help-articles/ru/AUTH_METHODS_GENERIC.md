...
Generic OAuth2 позволяет подключить любой OAuth2-совместимый провайдер идентификации к Exodus. Это полезно, когда вы хотите использовать провайдеры, для которых нет специальной интеграции (например, Authentik, Authelia, Zitadel, Google, Microsoft и др.).
...
| **Frontend Domain**   | Домен вашей панели Exodus без `https://` (например, `panel.example.com`)          |
...
Укажите список разрешённых email-адресов в настройках Exodus. Только пользователи с email из этого списка смогут войти.

### Вариант B: Использование Custom Claim

Если список **Allowed Emails** пуст, Exodus проверит наличие кастомного claim в токене:

| Ключ              | Значение |
| ----------------- | -------- |
| `exodusAccess` | `true`   |

Если токен пользователя содержит `exodusAccess: true`, он будет авторизован.
...
- Эндпоинт токена вашего провайдера доступен с сервера Exodus
