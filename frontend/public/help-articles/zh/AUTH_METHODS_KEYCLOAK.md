...
在令牌中添加自定义 claim `exodusAccess: true`：
...
您可以在 Exodus 设置中指定允许的邮箱地址列表，而不是配置 claim。只有邮箱在此列表中的用户才能登录。
...
| **Frontend Domain** | Exodus 面板域名，不含 `https://`（例如 `panel.example.com`）|
...
请确保满足以下条件之一：
- 用户通过 mapper 配置了 `exodusAccess: true` claim，或
- 用户的邮箱已添加到 Exodus 设置的 Allowed Emails 列表中
