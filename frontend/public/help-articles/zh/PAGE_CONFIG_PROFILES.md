...
在 Exodus 的配套设置中，配置文件（Config Profile） 代表一份完整的服务器配置， 用于 [Xray](https://xtls.github.io/en/config/) 内核.
...
目前，Exodus 支持以下协议： `VLESS`, `Trojan`, `Shadowsocks` (`chacha20-ietf-poly1305`, `2022-blake3-aes-256-gcm`, `aes-128-gcm`, `aes-256-gcm`), `Hysteria2`（仅用于客户端 Xray-Json 生成）。 并支持以下传输方式： `RAW (TCP)`, `XHTTP`, `Websocket`, `HTTPUpgrade`, `gRPC`, `KCP`.

需要注意的是，Exodus 也支持以下协议： `Tunnel`, `mixed(socks)`, `wireguard`, `http` - 但面板不会对它们进行任何处理，这些协议的用户管理功能将不可用。 此类 _入站（inbounds）_ 将按原样传递给 Xray，不会被修改。

对于主要协议 (`VLESS`, `Trojan`, `Shadowsocks`), Exodus 会自动管理服务器配置中的用户列表，无需额外手动操作。
...
默认情况下，Exodus 会自动为以下配置添加 `flow` 参数：  
