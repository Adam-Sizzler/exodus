...
4. Configure:
   - **Client ID**: `exodus` (or your preferred name)
...
Add a custom claim `exodusAccess: true` to the token:
...
Instead of configuring a claim, you can specify a list of allowed email addresses in the Exodus settings. Only users with emails in this list will be able to log in.
...
### "Email is not in the allowed list and exodusAccess claim is not present"
Make sure either:
- The user has the `exodusAccess: true` claim configured via mapper, OR
- The user's email is added to the Allowed Emails list in Exodus settings
