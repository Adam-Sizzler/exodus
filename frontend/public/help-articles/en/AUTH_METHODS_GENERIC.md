...
Generic OAuth2 allows you to connect any OAuth2-compatible identity provider to Exodus. This is useful when you want to use providers that don't have dedicated integration (like Authentik, Authelia, Zitadel, Google, Microsoft, etc.).
...
| **Frontend Domain**   | Your Exodus panel domain without `https://` (e.g., `panel.example.com`)            |
...
Specify a list of allowed email addresses in the Exodus settings. Only users with emails in this list will be able to log in.

### Option B: Using Custom Claim

If the **Allowed Emails** list is empty, Exodus will check for a custom claim in the token:

| Key               | Value  |
| ----------------- | ------ |
| `exodusAccess` | `true` |

If the user's token contains `exodusAccess: true`, they will be authorized.
...
- Your provider's token endpoint is accessible from your Exodus server
